...
!$OMP DO SCHEDULE (STATIC)
    DO I=1,TNDL
        DO K=1,IEXCHANGES(I)%MUCHTHEYNEED(1)
            ...
            IF (DG == 1) THEN
            IEXBOUNDHIS(I)%FACESOL(K,1:NOF_VARIABLES)=ILOCAL_RECON3(IEXCHANGES(I)%LOCALREF(K))%ULEFT_DG(1:NOF_VARIABLES,IEXCHANGEs(I)%SIDEtheyNEED(K),IEXCHANGES(I)%QTHEYNEED(k))
