...
SUBROUTINE RECONSTRUCT_DG(N)
    IMPLICIT NONE
    INTEGER,INTENT(IN)::N
    INTEGER::I_FACE, I_ELEM, I_QP, KMAXE, DG_RECONSTRUCT_ORDER, DG_TOTAL_DOFS, N_QP
	REAL,DIMENSION(:,:,:),ALLOCATABLE::U_C_VALDG !I_ELEM, NOF_VARIABLES, DG_DOFS

	KMAXE = XMPIELRANK(N)
	
	IF (DG == 1 .AND. DG_RECONSTRUCT == 1) THEN ! Reconstruct DG using LUO_LSQ_RECONSTRUCT
        DG_RECONSTRUCT_ORDER = IORDER + 1
        DG_TOTAL_DOFS = NUM_DG_DOFS + NUM_DG_RECONSTRUCT_DOFS - 1
        
        ALLOCATE(U_C_VALDG(KMAXE, NOF_VARIABLES, DG_TOTAL_DOFS + 1))
        
        DO I_ELEM = 1, KMAXE
            U_C_VALDG(I_ELEM,:,:) = LUO_LSQ_RECONSTRUCT(N, I_ELEM)
        END DO
    ELSE IF (DG == 1 .AND. DG_RECONSTRUCT == 0) THEN ! Don't use LUO_LSQ_RECONSTRUCT
        DG_RECONSTRUCT_ORDER = IORDER
        DG_TOTAL_DOFS = NUM_DG_DOFS - 1
        
        ALLOCATE(U_C_VALDG(KMAXE, NOF_VARIABLES, DG_TOTAL_DOFS + 1))
    
        DO I_ELEM = 1, KMAXE
            U_C_VALDG(I_ELEM,:,:) = U_C(I_ELEM)%VALDG(1,:,:)
        END DO
    END IF
    ...
    DO I_ELEM = 1, XMPIELRANK(N)
        ...
        DO I_FACE = 1, IELEM(N,I_ELEM)%IFCA
            DO I_QP = 1, QP_LINE_N
                ILOCAL_RECON3(I_ELEM)%ULEFT_DG(:, I_FACE, I_QP) = DG_SOL(N, I_ELEM, ILOCAL_RECON3(I_ELEM)%SURF_QPOINTS(I_FACE,I_QP,1), ILOCAL_RECON3(I_ELEM)%SURF_QPOINTS(I_FACE,I_QP,2), NOF_VARIABLES, DG_RECONSTRUCT_ORDER, DG_TOTAL_DOFS, U_C_VALDG(I_ELEM,:,:))
            END DO
        END DO
        
        ! Volume QP extrapolation
        ...
        END SELECT
        DO I_QP = 1, N_QP
            ILOCAL_RECON3(I_ELEM)%DG_VOL_SOL(:, I_QP) = DG_SOL(N, I_ELEM, QP_ARRAY(I_ELEM,I_QP)%X, QP_ARRAY(I_ELEM,I_QP)%Y, NOF_VARIABLES, DG_RECONSTRUCT_ORDER, DG_TOTAL_DOFS, U_C_VALDG(I_ELEM,:,:))
        END DO
    END DO
...
!> Prestores IELEM(N,I)%DELTA_XYZ, QP_ARRAY, SURF_QPOINTS, mass matrix
    ...
    DO I = 1, XMPIELRANK(N)    
        ...
        ALLOCATE(ILOCAL_RECON3(I)%DG_VOL_SOL(NOF_VARIABLES, N_QP))
    END DO
    ...
    DO I_ELEM = 1, KMAXE
        ...
        DO I_QP = 1, N_QP
            ...
            DO I_DOF = 1, IDEGFREE
                ...
                DO J_DOF = 1, IDEGFREE
                    MASS_MATRIX_CENTERS(N, I_ELEM, I_DOF+1, J_DOF+1) = MASS_MATRIX_CENTERS(N, I_ELEM, I_DOF+1, J_DOF+1) + BASIS_VECTOR(I_DOF) * BASIS_VECTOR(J_DOF) * QP_ARRAY(I_ELEM,I_QP)%QP_WEIGHT
                END DO
            END DO
        END DO
...
SUBROUTINE FACE_RECONSTRUCT_MATRIX(I_FACE, I_ELEM, LHS_MATRIX, RHS_DG_RECONSTRUCT)
!> @brief
!> This function appends to the LHS and RHS matrices for the Luo reconstruction
    INTEGER,INTENT(IN)::I_FACE, I_ELEM
    INTEGER::NEIGHBOR_INDEX, I_DG_RECONSTRUCT_DOF, FACE_INDEX_OFFSET, I_DIM
    REAL,DIMENSION(NUM_DG_RECONSTRUCT_DOFS + NUM_DG_DOFS)::BASIS_TEMP
    REAL,DIMENSION(NOF_VARIABLES, IELEM(N, I_ELEM)%IFCA * NUM_DG_DOFS, NUM_DG_RECONSTRUCT_DOFS),INTENT(INOUT)::LHS_MATRIX ! See eq. 3.19 of Luo 2012; NOF_VARIABLES
    REAL,DIMENSION(NOF_VARIABLES, IELEM(N, I_ELEM)%IFCA * NUM_DG_DOFS),INTENT(INOUT)::RHS_DG_RECONSTRUCT ! See eq. 3.19 of Luo 2012; NOF_VARIABLES, 
    
    NEIGHBOR_INDEX = IELEM(N,I_ELEM)%INEIGH(I_FACE)
    FACE_INDEX_OFFSET = (DIMENSIONA + 1) * (I_FACE - 1)
    ...
    DO I_DG_RECONSTRUCT_DOF = 1, NUM_DG_RECONSTRUCT_DOFS
        LHS_MATRIX(:,FACE_INDEX_OFFSET+1,I_DG_RECONSTRUCT_DOF) = BASIS_TEMP(DIMENSIONA+I_DG_RECONSTRUCT_DOF-1)
    END DO
    
    LHS_MATRIX(:,FACE_INDEX_OFFSET+2,1) = BASIS_TEMP(1)
    LHS_MATRIX(:,FACE_INDEX_OFFSET+2,DIMENSIONA+1) = BASIS_TEMP(2)
    LHS_MATRIX(:,FACE_INDEX_OFFSET+3,2) = BASIS_TEMP(2)
    LHS_MATRIX(:,FACE_INDEX_OFFSET+3,DIMENSIONA+1) = BASIS_TEMP(1)
    
    RHS_DG_RECONSTRUCT(:,FACE_INDEX_OFFSET+1) = DG_SOL(N, I_ELEM, IELEM(N, NEIGHBOR_INDEX)%XXC, IELEM(N,NEIGHBOR_INDEX)%YYC, NOF_VARIABLES, IORDER, IDEGFREE, U_C(NEIGHBOR_INDEX)%VALDG(1,:,:)) - DG_SOL(N, I_ELEM, IELEM(N, NEIGHBOR_INDEX)%XXC, IELEM(N,NEIGHBOR_INDEX)%YYC, NOF_VARIABLES, IORDER, IDEGFREE, U_C(I_ELEM)%VALDG(1,:,:))
    DO I_DIM = 1, DIMENSIONA
        RHS_DG_RECONSTRUCT(:,FACE_INDEX_OFFSET+I_DIM+1) = IELEM(N,I_ELEM)%DELTA_XYZ(I_DIM) / IELEM(N,NEIGHBOR_INDEX)%DELTA_XYZ(I_DIM) * U_C(I_ELEM)%VALDG(1,:,I_DIM+1) - U_C(NEIGHBOR_INDEX)%VALDG(1,:,I_DIM+1)
    END DO
END SUBROUTINE FACE_RECONSTRUCT_MATRIX

FUNCTION LUO_LSQ_RECONSTRUCT(N, I_ELEM)
!> @brief
...
!> REQUIRES: IELEM, U_C as globals
    ...
    INTEGER::I_FACE, I_VAR
    REAL,DIMENSION(NOF_VARIABLES, IELEM(N, I_ELEM)%IFCA * NUM_DG_DOFS, NUM_DG_RECONSTRUCT_DOFS)::LHS_MATRIX ! See eq. 3.19 of Luo 2012; NOF_VARIABLES
    REAL,DIMENSION(NOF_VARIABLES, IELEM(N, I_ELEM)%IFCA * NUM_DG_DOFS)::RHS_DG_RECONSTRUCT ! See eq. 3.19 of Luo 2012; NOF_VARIABLES, 
    REAL,DIMENSION(NOF_VARIABLES, NUM_DG_RECONSTRUCT_DOFS)::LUO_LSQ_RECONSTRUCT ! NUM_DG_RECONSTRUCT_DOFS
    
    LHS_MATRIX = ZERO
    
    IF (IELEM(N, I_ELEM)%INTERIOR == 0) THEN ! Element is interior
        DO I_FACE = 1, IELEM(N, I_ELEM)%IFCA
            CALL FACE_RECONSTRUCT_MATRIX(I_FACE, I_ELEM, LHS_MATRIX, RHS_DG_RECONSTRUCT)
        END DO
    ELSE IF (IELEM(N, I_ELEM)%INTERIOR == 1) THEN ! Element is not interior
!         DO I_FACE = 1,IELEM(N, I_ELEM)%IFCA
!             IF (IELEM(N, I_ELEM)%INEIGHB(I_FACE).EQ.N)THEN	!MY CPU ONLY
!                 IF (IELEM(N, I_ELEM)%IBOUNDS(I_FACE).GT.0)THEN	!CHECK FOR BOUNDARIES
!                     if (ibound(n,ielem(n,I_ELEM)%ibounds(I_FACE))%icode.eq.5)then	!PERIODIC IN MY CPU
!                         CALL FACE_RECONSTRUCT_MATRIX(I_FACE, I_ELEM, LHS_MATRIX, RHS_DG_RECONSTRUCT)
...
!                 IF (IELEM(N, I_ELEM)%IBOUNDS(I_FACE).GT.0)THEN	!CHECK FOR BOUNDARIES
!                     if (ibound(n,ielem(n,I_ELEM)%ibounds(I_FACE))%icode.eq.5)then	!PERIODIC IN OTHER CPU
!                         CRIGHT = 0
!                     END IF
!                 ELSE ! NOT BOUNDARY
!                     CRIGHT= 0
...
!         END DO
    ...
    DO I_VAR = 1, NOF_VARIABLES
        LUO_LSQ_RECONSTRUCT(I_VAR,:) = NORMAL_LSQ(LHS_MATRIX(I_VAR,:,:), RHS_DG_RECONSTRUCT(I_VAR,:))
    END DO
