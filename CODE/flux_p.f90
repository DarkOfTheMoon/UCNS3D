		    ...
		    DO L=1,IELEM(N,I)%IFCA
                ...
                DO NGP=1,IQP
                    IF (DG.EQ.1) THEN
                        ...
                        CRIGHT = DG_SOL(N, ILOCAL_RECON3(NEIGHBOR_INDEX)%QPOINTS(IELEM(N,I)%INEIGHN(L),NGP,1), ILOCAL_RECON3(NEIGHBOR_INDEX)%QPOINTS(IELEM(N,I)%INEIGHN(L),NGP,2), NOF_VARIABLES, IELEM(N,I)%IORDER, IELEM(N,I)%IDEGFREE, U_C(NEIGHBOR_INDEX)%VALDG(1,:,:))
                    ELSE
                        CLEFT(1)=ILOCAL_RECON3(I)%ULEFT(1,L,NGP)
                        CRIGHT(1)=ILOCAL_RECON3(IELEM(N,I)%INEIGH(L))%ULEFT(1,IELEM(N,I)%INEIGHN(L),NGP)
                    END IF
