...
!     WRITE(400+N,*) 'XD', SHAPE(RESHAPE((/ 1, 2, 3 /), (/ 3,1 /))), SHAPE(RESHAPE((/ 1, 2, 3 /), (/ 1,3 /)))
!     WRITE(400+N,*) RESHAPE((/ 1, 2, 3 /), (/ 1,3 /)) - RESHAPE((/ 1, 2, 3 /), (/ 3,1 /))
...
!> @brief
!> This function calculates the mass matrix for the element VEXTX:  0.222771778206677       0.211518513967366    0.251802460004030      VEXTY:  0.439008694688869       0.400724462066210 0.421177226361005      XXC:  0.228697584059357      YYC:  0.420303461038695
    IMPLICIT NONE
    INTEGER::I_QP, N_QP, N_DIM, N_DOFS, I_DOF, J_DOF
    REAL::CELL_VOLUME
    REAL,DIMENSION(2)::XCYC, DELTA_XY !N_DIM
!     REAL,DIMENSION(3)::QP_WEIGHT_TEST !N_QP
!     REAL,DIMENSION(2,6)::QP_TEST !N_DIM, N_QP
    REAL,DIMENSION(QP_TRIANGLE,5)::BASIS_VECTOR_TEST !N_QP, IDEGFREE
    REAL,DIMENSION(5,5)::MM_TEST !IDEGFREE, IDEGFREE
    REAL,DIMENSION(1,5,5)::INV_MM
    REAL,DIMENSION(5)::MM_TEST1 !IDEGFREE
    
    N_QP = QP_TRIANGLE; N_DIM = 2; N_DOFS = 5
...
!     QP_WEIGHT_TEST = (/ 1./3, 1./3, 1./3 /)
    VEXT(1,:) = (/ 0,0 /); VEXT(2,:) = (/ 1,0 /); VEXT(3,:) = (/ 0,1 /)
    CALL QUADRATURETRIANGLE(N,3)
    
    DELTA_XY = (/ 1./2, 1./2 /)
    
    DO I_QP = 1, N_QP
        BASIS_VECTOR_TEST(I_QP,:) = BASIS_REC2D(N,QPOINTS(1,I_QP),QPOINTS(2,I_QP),2,1,N_DOFS)
    END DO
    ...
    CALL COMPMASSINV(RESHAPE(MM_TEST,(/1,5,5/)),INV_MM,5,1)
...
!     1.164378853230725E-004  3.483781564527353E-005
!     1.180198033203196E-005 -1.600205160430905E-005 -1.638450552848604E-005
!     3.483781564527353E-005  1.095108304060082E-004 -8.192252764244054E-006
!     -1.870633804120096E-006 -3.200410320861758E-005  1.180198033203196E-005
!     -8.192252764244054E-006  2.583261933686550E-006 -1.967080106009016E-006
!     1.545806208340349E-006 -1.600205160430905E-005 -1.870633804120096E-006
!     -1.967080106009016E-006  2.285040857330715E-006  1.453844004915581E-006
!     -1.638450552848604E-005 -3.200410320861758E-005  1.545806208340349E-006
!     1.453844004915581E-006  9.718320071830323E-006
...
!     
!     DO I_QP = 1, N_QP
!         TAYLOR_BASIS_TEST(I_QP,:) = TAYLOR_BASIS(QP_TEST(:,I_QP), XCYC, DELTA_XY, QP_TEST, QP_WEIGHT_TEST, N_QP, N_DIM, N_DOFS, CELL_VOLUME)
!         WRITE(400+N,*) 'TEST_MASS_MATRIX BASIS', I_QP, TAYLOR_BASIS_TEST(I_QP,:)
!     END DO
!     
!     MM_TEST = 0
!     DO I_DOF = 1, N_DOFS
!         DO J_DOF = 1, N_DOFS
!             DO I_QP = 1, N_QP
!                 MM_TEST(I_DOF,J_DOF) = MM_TEST(I_DOF,J_DOF) + TAYLOR_BASIS_TEST(I_QP,I_DOF) * TAYLOR_BASIS_TEST(I_QP,J_DOF) * QP_WEIGHT_TEST(I_QP)
!             END DO
!         END DO
!     END DO
!     
...
!     DO I_DOF = 1, N_DOFS
!         DO I_QP = 1, N_QP
!             MM_TEST1(I_DOF) = MM_TEST1(I_DOF) + TAYLOR_BASIS_TEST(I_QP,I_DOF) * QP_WEIGHT_TEST(I_QP)
!         END DO
!     END DO
!     
!     WRITE(400+N,*) 'MM_TEST1', MM_TEST1 !Should be zero for every element
