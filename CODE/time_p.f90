...
DO I=1,KMAXE
    ...
    IF (DG == 1) THEN
        ...
        U_C(I)%VALDG(1,:,:)=U_C(I)%VALDG(2,:,:)-(DT*0.391752226571890*(RHS(I)%VALDG(:,:)*OOVOLUME))
    ELSE
        U_C(I)%VAL(2,1:NOF_VARIABLES)=U_C(I)%VAL(1,1:NOF_VARIABLES)
        U_C(I)%VAL(1,1:NOF_VARIABLES)=U_C(I)%VAL(2,1:NOF_VARIABLES)-(DT*0.391752226571890*(RHS(I)%VAL(1:NOF_VARIABLES)*OOVOLUME))
    END IF
...
DO I=1,KMAXE
  OOVOLUME=1.0D0/IELEM(N,I)%TOTVOLUME
    ...
    IF (DG == 1) THEN
        ...
        U_C(I)%VALDG(1,:,:) = 0.444370493651235 * U_C(I)%VALDG(2,:,:) + 0.555629506348765 * U_C(I)%VALDG(3,:,:) - 0.368410593050371 * DT * RHS(I)%VALDG(:,:) * OOVOLUME
    ELSE
        U_C(I)%VAL(3,1:NOF_VARIABLES)=U_C(I)%VAL(1,1:NOF_VARIABLES)
...
DO I=1,KMAXE
    ...
    IF (DG == 1) THEN
        ...
        U_C(I)%VALDG(1,:,:) = 0.620101851488403 * U_C(I)%VALDG(2,:,:) + 0.379898148511597 * U_C(I)%VALDG(4,:,:) - 0.251891774271694 * DT * RHS(I)%VALDG(:,:) * OOVOLUME
    ELSE
        U_C(I)%VAL(4,1:NOF_VARIABLES) = U_C(I)%VAL(1,1:NOF_VARIABLES)
...
DO I=1,KMAXE
    ...
    IF (DG == 1) THEN
        ...
        U_C(I)%VALDG(6,:,:) = - DT * RHS(I)%VALDG(:,:) * OOVOLUME
        U_C(I)%VALDG(1,:,:) = 0.178079954393132 * U_C(I)%VALDG(2,:,:) + 0.821920045606868 * U_C(I)%VALDG(5,:,:) - 0.544974750228521 * DT * RHS(I)%VALDG(:,:) * OOVOLUME
    ELSE
        U_C(I)%VAL(5,1:NOF_VARIABLES) = U_C(I)%VAL(1,1:NOF_VARIABLES)
        U_C(I)%VAL(6,1:NOF_VARIABLES) = - DT * RHS(I)%VAL(1:NOF_VARIABLES) * OOVOLUME
...
DO I=1,KMAXE
    ...
    IF (DG == 1) THEN
        U_C(I)%VALDG(1,1:NOF_VARIABLES,:) = (0.00683325884039 * U_C(I)%VALDG(2,1:NOF_VARIABLES,:)) + (0.517231671970585 * U_C(I)%VALDG(4,1:NOF_VARIABLES,:)) + (0.12759831133288 * U_C(I)%VALDG(5,1:NOF_VARIABLES,:)) + (0.34833675773694 * U_C(I)%VALDG(1,1:NOF_VARIABLES,:)) + (0.08460416338212 * U_C(I)%VALDG(6,1:NOF_VARIABLES,:)) - (0.22600748319395 * DT * RHS(I)%VALDG(1:NOF_VARIABLES,:) * OOVOLUME)
    ELSE
